		...
		\subsection*{Task 1.5 : Cluster Validity Measure}
			...
			\begin{figure}[H]
				...
				\begin{subfigure}{0.4\textwidth}
					...
					\caption{Counting Loop Start: Because of K-Means is an heuristic algorithm, it makes sense to be analysed when averaged in a series of runs, we will run for $n=100$)}
					\label{fig:first}
				\end{subfigure}
				\hfill
				\begin{subfigure}{0.4\textwidth}
				...
				\begin{subfigure}{0.4\textwidth}
					...
					\caption{Numeric Outliers: In order to extract meaningfull box plots, we need to eliminate outliers(very bad initializations), This node replaces the extremes with the closest permitted value}
					\label{fig:first}
				\end{subfigure}
					\label{fig:figures}
			\end{figure}
