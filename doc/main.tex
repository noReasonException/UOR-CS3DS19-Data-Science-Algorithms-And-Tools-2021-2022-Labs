		...
		\subsection*{Task 1.2 : Generate plot2}
			...
			\subsubsection{K-Means Algorithm}
				...
				\begin{itemize}[noitemsep,topsep=0pt]
					\item For every datapoint in set, calculate the distances of the given point from the $k$ estimated cluster centroids
					\item Assign the datapoint into the closest cluster.
					\item When the $k$ clusters have been calculated, calculate the cluster's centroids, and set these centroids as the best estimated cluster centroids
					\item Repeat untill accuracy is acceptable(Accuracy gets calculated with SSE, SSE is explained on Task 1.5)
				\end{itemize}
